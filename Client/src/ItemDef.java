...
public final class ItemDef {
	...
	public static void unpackConfig(StreamLoader streamLoader) {
		...
		stream289 = new Stream(StreamLoader.read(new File(signlink.findcachedir() + "obj289.dat")));
		stream377 = new Stream(streamLoader.getDataForName("obj.dat"));
		stream474 = new Stream(StreamLoader.read(new File(signlink.findcachedir() + "obj474.dat")));
		
		Stream streamIdx377 = new Stream(streamLoader.getDataForName("obj.idx"));
		totalItems = streamIdx377.readUnsignedWord();
		streamIndices377 = new int[totalItems];
		int i = 2;
		...
		Stream streamIdx289 = new Stream(StreamLoader.read(new File(signlink.findcachedir() + "obj289.idx")));
		totalItems = streamIdx289.readUnsignedWord();
		streamIndices289 = new int[totalItems];
		i = 2;
		for (int j = 0; j < totalItems; j++) {
		...
		Stream streamIdx474 = new Stream(StreamLoader.read(new File(signlink.findcachedir() + "obj474.idx")));
		totalItems = streamIdx474.readUnsignedWord();
		streamIndices474 = new int[totalItems];
		i = 2;
		for (int j = 0; j < totalItems; j++) {
			streamIndices474[j] = i;
			i += streamIdx474.readUnsignedWord();
		}
	...
	public static ItemDef forID(int i) {
		...
		cacheIndex = (cacheIndex + 1) % 10; // %10
		ItemDef itemDef = cache[cacheIndex];
		
		if(i < streamIndices289.length && (i < 1511 || 1522 < i)) {
			stream289.currentOffset = streamIndices289[i];
			itemDef.id = i;
			itemDef.setDefaults();
			itemDef.readValues(stream289);
		} else if(i < streamIndices377.length && i != 6570 && (i < 1511 || 1522 < i)) {
			stream377.currentOffset = streamIndices377[i];
			itemDef.id = i;
			itemDef.setDefaults();
			itemDef.readValues(stream377);
		} else {
	...
	private static int[] streamIndices377;
	private static int[] streamIndices289;
