...
public final class TextDrawingArea extends DrawingArea {
	...
	public void method385(int colour, String s, int j, int l) {
		if (s == null)
			return;
		j -= anInt1497;
	...
	private void method392(byte abyte0[], int i, int j, int k, int l, int colour) {
		...
		if (!(k <= 0 || l <= 0)) {
			method393(DrawingArea.pixels, abyte0, colour, i2, j1, k, l, k1, l1);
		}
	}

	private void method393(int ai[], byte abyte0[], int colour, int j, int k, int l, int i1, int j1, int k1) {
		int l1 = -(l >> 2);
		l = -(l & 3);
