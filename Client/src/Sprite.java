...
public final class Sprite extends DrawingArea {
	...
	public Sprite(StreamLoader streamLoader, String s, int i) {
		...
		if (i1 == 0) {
			for (int k1 = 0; k1 < j1; k1++) {
				try {
				int val = ai[stream.readUnsignedByte()];
				myPixels[k1] = val;
				} catch(Exception e) {}
			}
	...
	public void drawAdvancedSprite(int posX, int posY) {
		...
		if (width > 0 && height > 0) {
			drawToContainer(width, height, pixelOffset, myPixels, spriteScanSize, containerPixel, DrawingArea.pixels,
					containerScanSize);
		}
	}

	private void drawToContainer(int width, int height, int spritePixel, int spriteData[], int spriteScanSize,
			int containerPixel, int containerData[], int containerScanSize) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				if (argb != 0) {
					...
					containerData[containerPixel++] = ((argb & 0xff00ff) * alpha
							+ (originalColor & 0xff00ff) * transparency & 0xff00ff00)
							+ ((argb & 0xff00) * alpha + (originalColor & 0xff00) * transparency & 0xff0000) >> 8;
				} else {
					containerPixel++;
				}
			}
