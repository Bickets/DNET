	...
	public static void unpack(StreamLoader streamLoader, TextDrawingArea textDrawingAreas[],
		...
		while (stream.currentOffset < stream.buffer.length) {
			...
			if (rsInterface.type == 0) {
				...
				if(k == DashboardMusic.SCROLL_PANE) {
        ...
        rsInterface.aString228 = stream.readString();
			...
			if(rsInterface.parentID == DashboardMusic.MUSIC_TAB && rsInterface.type == 4 && !DashboardMusic.textAllowed(k)) {
			  //rsInterface.message = "" + k;
			  //interfaceCache[k] = rsInterface;
			} else {
			  interfaceCache[k] = rsInterface;
			}
		}
