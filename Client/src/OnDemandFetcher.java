...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
		...
		try {
			if (socket == null) {
				...
				if (l - openSocketTime < 4000L)
					return;
				openSocketTime = l;
				socket = clientInstance.openSocket(5555 + Client.portOff);
	...
	public static byte[] unGzip(File file) {
		byte[] inputBuffer = new byte[650000];
		int i = 0;
		try {
			GZIPInputStream gzipinputstream = new GZIPInputStream(new ByteArrayInputStream(StreamLoader.read(file)));
			do {
				if (i == inputBuffer.length)
					throw new RuntimeException("buffer overflow!");
				int k = gzipinputstream.read(inputBuffer, i, inputBuffer.length - i);
				if (k == -1)
					break;
				i += k;
			} while (true);
		...
		System.arraycopy(inputBuffer, 0, buffer, 0, i);
		return buffer;
	}
