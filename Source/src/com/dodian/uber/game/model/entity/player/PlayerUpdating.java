...
import com.dodian.uber.game.model.entity.EntityUpdating;
...
public class PlayerUpdating extends EntityUpdating<Player> {
  ...
  public static void updateLocalPlayerMovement(Player player, Stream stream) {
    ...
    if (player.didTeleport() || player.didMapRegionChange()) {
      ...
      System.out.println("x: "+player.getCurrentX()+", y:"+player.getCurrentY());
      return;
    }
  ...
  public void appendAnimationRequest(Player player, Stream stream) {
    stream.writeWordBigEndian((player.getAnimationId() == 0) ? 65535 : player.getAnimationId());
  ...
  public static void appendPlayerAppearance(Player player, Stream str) {
    ...
    if (player.isNpc() == false) {
      ...
      if (player.getEquipment()[Equipment.Slot.WEAPON.getId()] > 1) {
      ...
      if (player.getEquipment()[Equipment.Slot.SHIELD.getId()] > 1) {
      ...
      if (!Server.itemManager.isFullHelm(player.getEquipment()[Equipment.Slot.HEAD.getId()])) {
      ...
      if (!Server.itemManager.isFullHelm(player.getEquipment()[Equipment.Slot.HEAD.getId()]) && (player.getPlayerLook()[0] != 1)) {
    ...
    playerProps.writeByte(player.getPlayerLook()[1]); // hair color
    playerProps.writeByte(player.getPlayerLook()[2]); // torso color.
    playerProps.writeByte(player.getPlayerLook()[3]); // leg color
    playerProps.writeByte(player.getPlayerLook()[4]); // feet color
    playerProps.writeByte(player.getPlayerLook()[5]); // skin color (0-6)
  ...
  public void appendPrimaryHit(Player player, Stream stream) {
    try {
      ...
      if (player.getHitDiff() == 0) {
        stream.writeByteA(0);
      } else if (player.getHitDiff() > 0 && !player.isCrit()) {
        stream.writeByteA(1); // 0: red hitting - 1: blue hitting
      } else {
        stream.writeByteA(3);
      }
