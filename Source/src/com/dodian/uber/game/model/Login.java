...
public Login() {
  ...
  public synchronized void logTrade(int p1, int p2, CopyOnWriteArrayList<GameItem> items,
    ...
    try {
      ...
      System.out.println("Trade " + id + " logged!");
    } catch (Exception e) {
  ...
  public synchronized void sendSession(int dbId, int clientPid, long elapsed, String connectedFrom) {
    try {
      Database.statement.executeUpdate("INSERT INTO uber3_sessions SET dbid=" + dbId + ", duration=" + elapsed
          + ", hostname='" + connectedFrom + "'");
    } catch (Exception e) {
      System.out.println(e.getMessage());
      e.printStackTrace();
    }
