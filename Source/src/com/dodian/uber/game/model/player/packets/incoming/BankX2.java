...
public class BankX2 implements Packet {
  ...
  public void ProcessPacket(Client client, int packetType, int packetSize) {
    ...
    if (client.XinterfaceID == 5064) { // remove from bag to bank
      ...
      client.fromBank(client.bankItems[client.XremoveSlot], client.XremoveSlot, EnteredAmount);
    } else if (client.XinterfaceID == 3322 && client.inDuel) { // remove from
                                                               // bag
      // to duel window
      client.stakeItem(client.XremoveID, client.XremoveSlot, EnteredAmount);
    } else if (client.XinterfaceID == 6669 && client.inDuel) { // remove from
                                                               // duel
      // window
      client.fromDuel(client.XremoveID, client.XremoveSlot, EnteredAmount);
    } else if (client.XinterfaceID == 3322 && client.inTrade) { // remove from
                                                                // bag to trade
      // window
    ...
    } else if (client.XinterfaceID == 3415 && client.inTrade) { // remove from
                                                                // trade window
